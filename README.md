...
例如同時上傳 JPG 與 GIF 時，JPG 可以成功處理，GIF 會因格式不支援被標示為失敗。
...
| 點擊一鍵下載 | 下載所有成功處理的圖片 |
| 未選圖片直接送出 | 前台會顯示瀏覽器原生選檔提示；API 會回傳「請至少選擇一張圖片」
| 上傳不支援格式 | 顯示格式不支援提示 |
...
若在前台操作，瀏覽器會先顯示原生選檔提示，實際文字可能會因瀏覽器不同而略有差異。
...
本專案目前已完成圖片批次上傳、格式轉換、壓縮品質設定、結果顯示與下載功能。
