...
例如使用 API 測試時，同時上傳 JPG 與 GIF，JPG 可以成功處理，GIF 會因格式不支援被標示為失敗。
...
若使用 Postman 測試 API，且沒有帶入 `images` 欄位，後端會回傳：
...
11. 示範錯誤情境，例如拖曳不支援格式 GIF
12. 示範未選檔案時的前端自訂提示
13. 示範超過 10 張圖片時的張數限制提示
...
| 成員 | AI 協助內容 |
| --- | --- |
| JinL | 使用 AI 分層分析合併後的功能問題，逐步補齊前端介面缺失，並在改動最小化的前提下確認可行方案。 |
| Mori | 使用 AI 協助討論批量處理流程與可行方案，最終採用 `Promise.all`，等所有圖片處理完成後再統一整理結果。 |
| June | 使用 Claude Code 協助檢查圖片驗證機制，分析可能的不當操作或安全風險。 |
